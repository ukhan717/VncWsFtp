...
static void _callback_CGI_GetDetails(WEBS_OUTPUT* pOutput, const char* sParameters) {
  ...
  if (strcmp(acPara, "embOSMPU") == 0) {
    ...
    IP_WEBS_SendString(pOutput, "</div>");
  } else if (strcmp(acPara, "embOSIP") == 0) {
    IP_WEBS_SendString(pOutput, "<div class=\"textBlock\">");
    IP_WEBS_SendString(pOutput, "<h2>embOS/IP - Ideal for Any Network Application</h2>");
    ...
    IP_WEBS_SendString(pOutput, "<br><br><a href=\"https://www.segger.com/products/connectivity/embosip/\" target=\"_blank\">https://www.segger.com/products/connectivity/embosip/</a> <span class=\"hint\"><br>(external link, an internet connection is required)</span>");
    IP_WEBS_SendString(pOutput, "</div>");
  ...
  if (sValue != NULL) {
    ...
    if (v == 1) {
      if (_DHCPOnOff == 0) {                      // Is DHCP client disabled ?
        ...
        IP_DHCPC_Activate(0, NULL, NULL, NULL);   // Enable DHCP client.
      }
    } else {
    ...
    _DHCPOnOff = _atoi(sValue);                   // Update DHCP state.
  }
}
