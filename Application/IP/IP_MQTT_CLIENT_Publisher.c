...
Purpose : Sample program for embOS & embOS/IP demonstrating an MQTT publisher.
*/
...
static void _MQTT_Client(void) {
  ...
  do {
    ...
    while (1) {
      ...
      IP_MQTT_CLIENT_LOG(("APP:   Topic  : \"%s\"", Publish.sTopic));
      IP_MQTT_CLIENT_LOG(("APP:   Payload: \"%s\"", Publish.pData));
      OS_Delay(1000);
    }
    //
    // Disconnect.
