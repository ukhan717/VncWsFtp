...
// Setup Flags for CDC
//
#define USBH_CDC_IGNORE_INT_EP           (1u << 0)          // Ignore, that an Interrupt EP is available. It can also be used where no Interrupt EP is available.
#define USBH_CDC_DISABLE_INTERFACE_CHECK (1u << 1)          // Disable, that the data interface no. in the ACM interface descriptor is not valid and though is not checked.
